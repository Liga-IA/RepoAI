...
Agent agent(settings.get_belief_base(), settings.get_event_base(),
            settings.get_plan_base(), settings.get_intention_base());
...
bool action_request_cooperation_on() {
  ...
  if (espnow_manager.find_neighbor_by_capability(CAP_ACTUATOR_LED,
                                                 &led_neighbor)) {
    CooperationAction_t action_request;
    action_request.action_type = 1; // 1 = Turn ON LED
    return espnow_manager.send_cooperation_request(led_neighbor.mac,
                                                   &action_request);
  }
  return false;
}
...
// --- ESP-NOW Callback ---
void on_message_received(const uint8_t *mac_addr, const uint8_t *data,
                         int len) {
  NetworkMessage_t msg;
  if (len > sizeof(NetworkMessage_t))
    return;
  memcpy(&msg, data, len);
  if (memcmp(msg.sender_mac, espnow_manager.get_agent_mac(), 6) == 0)
    return;
  ...
  if (msg.type == MSG_DISCOVERY) {
    ...
    if (received_action.action_type == 3) { // 3 = Sound Alarm
      Serial.println(
          "[NETWORK] ALARM request received! Generating BDI event...");
      settings.get_event_base()->add_event(
          Event(EventOperator::BELIEF_ADDITION, Proposition(1)));
    }
  }
}
...
void setup() {
  ...
  uint8_t capabilities = CAP_SENSOR_LIGHT | CAP_ACTUATOR_BUZZER |
                         CAP_SENSOR_TEMP_HUMID | CAP_SENSOR_TEMP_ANALOG |
                         CAP_SENSOR_POT; // CAP_SENSOR_IR temporarily removed
  if (!espnow_manager.initialize("CentralAgent", capabilities,
                                 on_message_received)) {
    while (1)
      delay(1000);
  }
...
void loop() {
  ...
  if (current_time - last_sensor_read_time > 2000) { // Reading every 2 seconds
    ...
    if (is_dark != last_ldr_state) {
      ...
      if (is_dark) {
        ...
        settings.get_event_base()->add_event(
            Event(EventOperator::BELIEF_ADDITION, Proposition(0)));
      } else {
        Serial.println("[EVENT] Belief deleted: low_light (false)");
        settings.get_event_base()->add_event(
            Event(EventOperator::BELIEF_DELETION, Proposition(0)));
      }
    }
    ...
    if (is_pot_high != last_pot_high_state) {
      ...
      if (is_pot_high) {
        ...
        settings.get_event_base()->add_event(
            Event(EventOperator::BELIEF_ADDITION, Proposition(8)));
      } else {
        Serial.println("[EVENT] Belief deleted: pot_is_high (false)");
        settings.get_event_base()->add_event(
            Event(EventOperator::BELIEF_DELETION, Proposition(8)));
      }
    }
    ...
    if (is_hot != last_hot_state) {
      ...
      if (is_hot) {
        ...
        settings.get_event_base()->add_event(
            Event(EventOperator::BELIEF_ADDITION, Proposition(5)));
      } else {
        Serial.println("[EVENT] Belief deleted: is_hot (false)");
        settings.get_event_base()->add_event(
            Event(EventOperator::BELIEF_DELETION, Proposition(5)));
      }
    }
    ...
    if (!isnan(dht_humidity) && !isnan(dht_temp)) {
      ...
      if (is_humid != last_humid_state) {
        ...
        if (is_humid) {
          ...
          settings.get_event_base()->add_event(
              Event(EventOperator::BELIEF_ADDITION, Proposition(6)));
        } else {
          Serial.println("[EVENT] Belief deleted: is_humid (false)");
          settings.get_event_base()->add_event(
              Event(EventOperator::BELIEF_DELETION, Proposition(6)));
        }
      }
    } else {
  ...
  if (irrecv.decode(&results)) {
    ...
    settings.get_event_base()->add_event(Event(EventOperator::BELIEF_ADDITION,
  Proposition(7)));
  ...
  */
  ...
  if (current_time - last_belief_sync_time > 1000) {
    ...
    bool neighbor_found =
        espnow_manager.find_neighbor_by_capability(CAP_ACTUATOR_LED, &neighbor);

    if (neighbor_found && !has_neighbor_belief) {
      settings.get_event_base()->add_event(
          Event(EventOperator::BELIEF_ADDITION, Proposition(2)));
      settings.get_event_base()->add_event(
          Event(EventOperator::BELIEF_ADDITION, Proposition(3)));
      has_neighbor_belief = true;
    } else if (!neighbor_found && has_neighbor_belief) {
      settings.get_event_base()->add_event(
          Event(EventOperator::BELIEF_DELETION, Proposition(2)));
      settings.get_event_base()->add_event(
          Event(EventOperator::BELIEF_DELETION, Proposition(3)));
