...
As explained in (link to conceptual), we have the following characteristics of k:
•	k, when small, tends to cause overfitting
•	k, when large, tends to cause underfitting
•	k, when even, can lead to ties in classification problems and, consequently, inaccuracies, as already explained in (link)
...
> Therefore, we can understand bias via the same example used in “Underfitting and high k values” in the conceptual section (link).
...
> Therefore, we can understand variance via the same example used in “Overfitting and low k values” in the conceptual section (link).
...
However, we don't always have a large enough dataset to be able to do these two types of separate tests.
...
Como explicado em (link para conceitual), temos as seguintes características de k:
